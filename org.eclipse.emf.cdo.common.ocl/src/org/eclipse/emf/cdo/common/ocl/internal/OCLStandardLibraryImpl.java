 ...
 * </copyright>
 *
 * $Id: OCLStandardLibraryImpl.java,v 1.2 2009-01-04 17:56:36 estepper Exp $
 */
...
import org.eclipse.emf.cdo.common.ocl.EcoreEnvironmentFactory;
...
public final class OCLStandardLibraryImpl
	...
	public static final EObject OCL_INVALID = stdlibPackage
	...
	private static EPackage init() {
		...
		try {
			...
			res = OCLEcorePlugin.getEcoreResourceFactory().createResource(
				load.getURI());
			...
			OCL_ELEMENT = stdlibPackage
				.getEClassifier(ElementType.SINGLETON_NAME);
			OCL_BOOLEAN = stdlibPackage
				.getEClassifier(PrimitiveType.BOOLEAN_NAME);
			OCL_INTEGER = stdlibPackage
				.getEClassifier(PrimitiveType.INTEGER_NAME);
			OCL_UNLIMITED_NATURAL = stdlibPackage
				.getEClassifier(PrimitiveType.UNLIMITED_NATURAL_NAME);
			OCL_REAL = stdlibPackage.getEClassifier(PrimitiveType.REAL_NAME);
			OCL_STRING = stdlibPackage
				.getEClassifier(PrimitiveType.STRING_NAME);
			OCL_VOID = stdlibPackage.getEClassifier(VoidType.SINGLETON_NAME);
			OCL_MESSAGE = stdlibPackage
				.getEClassifier(MessageType.SINGLETON_NAME);
			...
			OCL_TYPE = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage
				.getEClassifiers(), OCLPackage.Literals.TYPE_TYPE);

			OCL_SET = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage
				.getEClassifiers(), OCLPackage.Literals.SET_TYPE);
			OCL_ORDERED_SET = (EClassifier) EcoreUtil.getObjectByType(
				stdlibPackage.getEClassifiers(),
				OCLPackage.Literals.ORDERED_SET_TYPE);
			OCL_BAG = (EClassifier) EcoreUtil.getObjectByType(stdlibPackage
				.getEClassifiers(), OCLPackage.Literals.BAG_TYPE);
			OCL_SEQUENCE = (EClassifier) EcoreUtil.getObjectByType(
				stdlibPackage.getEClassifiers(),
				OCLPackage.Literals.SEQUENCE_TYPE);

			// don't use EcoreUtil because the other collection types would
			// match
			...
			EPackage.Registry.INSTANCE.put(stdlibPackage.getNsURI(),
				stdlibPackage);
	...
	private static EPackage build() {
		...
		Environment<?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?> old = Environment.Registry.INSTANCE
			.getEnvironmentFor(OCLPackage.Literals.ANY_TYPE);

		stdlibPackage = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
			.createEPackage();
		...
		Resource res = factory.createResource(URI.createURI(stdlibPackage
			.getNsURI()));
		res.getContents().add(stdlibPackage);

		Environment.Registry.INSTANCE.deregisterEnvironment(old);
		EcoreEnvironment env = (EcoreEnvironment) EcoreEnvironmentFactory.INSTANCE
			.loadEnvironment(res);
		Environment.Registry.INSTANCE.registerEnvironment(env);

		OCL_ANY = OCLFactory.eINSTANCE.createAnyType();
		OCL_ELEMENT = OCLFactory.eINSTANCE.createElementType();
		OCL_BOOLEAN = OCLFactory.eINSTANCE.createPrimitiveType();
		OCL_BOOLEAN.setName(PrimitiveType.BOOLEAN_NAME);
		OCL_INTEGER = OCLFactory.eINSTANCE.createPrimitiveType();
		OCL_INTEGER.setName(PrimitiveType.INTEGER_NAME);
		OCL_UNLIMITED_NATURAL = OCLFactory.eINSTANCE.createPrimitiveType();
		OCL_UNLIMITED_NATURAL.setName(PrimitiveType.UNLIMITED_NATURAL_NAME);
		OCL_REAL = OCLFactory.eINSTANCE.createPrimitiveType();
		OCL_REAL.setName(PrimitiveType.REAL_NAME);
		OCL_STRING = OCLFactory.eINSTANCE.createPrimitiveType();
		OCL_STRING.setName(PrimitiveType.STRING_NAME);
		OCL_VOID = OCLFactory.eINSTANCE.createVoidType();
		OCL_MESSAGE = OCLFactory.eINSTANCE.createMessageType();

		INVALID = OCLFactory.eINSTANCE.createInvalidType();

		OCL_T = OCLFactory.eINSTANCE.createAnyType();
		OCL_T.setName("T"); //$NON-NLS-1$
		OCL_T2 = OCLFactory.eINSTANCE.createAnyType();
		...
		OCL_ORDERED_SET = (EClassifier) OCLFactoryImpl.INSTANCE
			.createOrderedSetType(OCL_T);
		OCL_BAG = (EClassifier) OCLFactoryImpl.INSTANCE.createBagType(OCL_T);
		OCL_SEQUENCE = (EClassifier) OCLFactoryImpl.INSTANCE
			.createSequenceType(OCL_T);
		OCL_COLLECTION = (EClassifier) OCLFactoryImpl.INSTANCE
			.createCollectionType(OCL_T);

		STATE = OCLFactory.eINSTANCE.createElementType();
		STATE.setName("State"); //$NON-NLS-1$
		OCL_EXPRESSION = OCLFactory.eINSTANCE.createElementType();
		...
		operations.addAll(OCLStandardLibraryUtil
	...
	private static EClass register(EClassifier stdType) {
		EClass result = (stdType instanceof EClass)
			? (EClass) stdType
			: null;
	...
	public static EClass createShadowClass(EClassifier classifier) {
		...
		EClass result = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
			.createEClass();
		result.setName(classifier.getName() + "_Class"); //$NON-NLS-1$

		EAnnotation ann = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
			.createEAnnotation();
	...
	/**
	 * Finds the shadow class to contain features defined for the specified OCL
	 * <code>type</code>, if it already exists.
	 * 
	 * @param classifier
	 *            an Ecore classifier
	 * @param pkg
	...
	/**
	 * Obtains the existing operations of the specified type, stored in it or in
	 * a shadow class. <b>Note</b> that this method returns <code>null</code>,
	 * not an empty list, if none are found.
	 * 
	 * @param type
	...
	public static EList<EOperation> getExistingOperations(EClassifier type) {
		...
		return (result == null)
			? ECollections.<EOperation> emptyEList()
			: result;
	}
	 ...
	 * 
	 * @param operations
	 *            operations to designate as iterators
	 */
	...
	/**
	 * Selects from the specified operations those that are collection
	 * iterators.
	 * 
	 * @param operations
	 *            operations
