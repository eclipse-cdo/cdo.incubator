 ...
 * </copyright>
 *
 * $Id: UMLReflectionImpl.java,v 1.2 2009-01-04 17:56:36 estepper Exp $
 */
...
public class UMLReflectionImpl
	...
	public String getDescription(Object namedElement) {
		if (namedElement instanceof ETypedElement) {
			...
			return (type == null)
				? OCLStandardLibraryImpl.INSTANCE.getOclVoid().getName()
				: type.getName();
	...
	public String getName(Object namedElement) {
		return (namedElement instanceof ENamedElement)
			? ((ENamedElement) namedElement).getName()
			: null;
	}
	...
	public EClassifier getOCLType(Object metaElement) {
		...
		if (metaElement == EcorePackage.Literals.ETYPED_ELEMENT__UPPER_BOUND) {
			...
			if (isMany(typedElement)) {
				result = getOCLCollectionType(result, typedElement.isOrdered(),
	...
	public Collection<? extends EClassifier> getAllSupertypes(
	...
	public EClassifier getOwningClassifier(Object feature) {
		if (feature instanceof EOperation) {
			...
			return OCLStandardLibraryImpl
				.getOwner((EStructuralFeature) feature);
		}
	...
	public EOperation createOperation(String name, EClassifier resultType,
		...
		EOperation result = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
			.createEOperation();
		result.setName(name);
		...
		for (String pname : paramNames) {
			EParameter param = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
				.createEParameter();
	...
	public EStructuralFeature createProperty(String name, EClassifier resultType) {
		...
		if (resultType instanceof EClass) {
			result = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
				.createEReference();
		} else {
			result = org.eclipse.emf.ecore.EcoreFactory.eINSTANCE
				.createEAttribute();
	...
	/**
	 * Obtains the association class that the specified reference represents, as
	 * an {@link EClass}, if any. By default, it is assumed that nothing is an
	...
	public CallOperationAction createCallOperationAction(EOperation operation) {
		CallOperationAction result = OCLFactory.eINSTANCE
	...
	public SendSignalAction createSendSignalAction(EClassifier signal) {
		SendSignalAction result = OCLFactory.eINSTANCE.createSendSignalAction();
	...
	public Constraint createConstraint() {
		return OCLFactory.eINSTANCE.createConstraint();
	}
	...
	public ExpressionInOCL<EClassifier, EParameter> createExpressionInOCL() {
		return OCLFactory.eINSTANCE.createExpressionInOCL();
	}
	...
	/**
	 * Gets the qualifiers of an association end. By default, it is assumed that
	 * properties never have qualifiers because Ecore does not directly support
	 * this concept.
	 * 
	 * @param property
	...
	public EClassifier getCommonSuperType(EClassifier type1, EClassifier type2) {
		...
		if (type1 instanceof EClass && type2 instanceof EClass) {
			...
			List<EClass> allSuperAndSelf1 = new ArrayList<EClass>(eClass1
				.getEAllSuperTypes());
			allSuperAndSelf1.add(eClass1);

			List<EClass> allSuperAndSelf2 = new ArrayList<EClass>(eClass2
				.getEAllSuperTypes());
	 ...
	 * 
	 * @param dataType
	 *            a data type in the EMF metamodel
	 ...
	 * </ul>
	 * <p>
	 * Note that the collection's element <code>type</code> is mapped to an OCL
	 * primitive type where possible.
	 * </p>
	 * 
	 * @param type
	 *            the type to get the corresponding OCL collection type for
	 * @param isOrdered
	 *            whether the OCL type should be ordered
	 * @param isUnique
	 ...
	 */
	EClassifier getOCLCollectionType(EClassifier type, boolean isOrdered,
			boolean isUnique) {
		EClassifier resultType = type;
		...
		if (isOrdered) {
			if (isUnique) {
				resultType = (EClassifier) oclFactory
					.createOrderedSetType(resultType);
			} else {
				resultType = (EClassifier) oclFactory
					.createSequenceType(resultType);
			}
		} else {
			if (isUnique) {
	...
	/**
	 * Custom is-many determination to assume that XSD unspecified
	 * multiplicities are many.
	 * 
	 * @param typedElement
	 *            a typed element
	 * 
	 ...
	 * (recursively) of the specified <tt>descendent</tt>. This utility is like
	 * the similar {@link EcoreUtil} operation, except that it does not require
	 * the arguments to be {@link InternalEObject}s.
	 * 
	 * @param ancestor
	 *            a prospective ancestor element
	 * @param descendant
